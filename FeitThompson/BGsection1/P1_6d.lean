...
/-- **B (AXIOM)** — the meet half: in the abelian / coprime setting,
`⁅G,A⁆ ⊓ C_G(A) = ⊥`. This is the "TI" (trivial intersection)
content of B & G 1.6(d). Coq combines `coprime_abel_cent_TI` (from
`finmod.v`) with `setICA` to land this; the underlying MathComp lemma
lives outside `BGsection1.v`. Deferred. -/
axiom inf_bot
...
theorem coprime_abelian_cent_dprod
  ...
  ⟨BranchA_sup.cent_prod_top A hNorm hCoprime hAbel,
   BranchB_inf.inf_bot A hNorm hCoprime hAbel⟩
