# Online requests — ✅ ALL RESOLVED (retired 2026-06-03, host session)

> **This is the renamed/retired `ON-LINE-REQUEST.md`** (renamed to clear the 📚 badge per the
> Fulfiller protocol). Both asks below are answered in
> **`ON-LINE-FINDINGS-2026-06-03-burnside-endgame.md`**:
> - **Request 1** (trivial multiplicity `T = 1`): mathlib has NO packaged lemma (no "linear
>   character", no `#{dᵢ=1}=|Gᵃᵇ|`); the bare `exists_algEquiv_pi_matrix_of_isAlgClosed` carries no
>   per-factor / non-iso data (that lives in `Isotypic.lean`). **Recommended: the counting-free
>   augmentation route (Option D)** via `MonoidAlgebra.lift` + `MonoidAlgebra.algHom_ext`.
> - **Request 2** (matrix-module irreducibility transfer): **fully in mathlib — drop both Aristotle
>   jobs.** `IsSimpleModule (Module.End R M) M` (`SimpleModule/Basic.lean:536`) +
>   `LinearMap.isSimpleModule_iff_of_bijective` (`:97`, the `RingHomSurjective` transfer).
>
> The box recreates a fresh `ON-LINE-REQUEST.md` for its next open-web ask. Original request content
