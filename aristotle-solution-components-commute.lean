...
/-!
# Target: distinct components of a finite group commute (Aschbacher 31.4)

Self-contained statement. `IsSubnormal`, `IsQuasisimple`, `IsComponent` are
defined inline (mathlib has no notion of subnormal subgroup or quasisimple group).
Wielandt's join theorem is provided as `isSubnormal_sup` (a genuine theorem for
finite groups; supplied as an axiom you MAY use).

GOAL: fill the `sorry` in `components_commute`.

Mathematical route (Aschbacher, *Finite Group Theory*, 31.4): a component `L` and a
subnormal subgroup `H` satisfy `L ≤ H` or `⁅L, H⁆ = 1`. Apply with `H = M` (a
component, hence subnormal): since `L ≠ M` and a component contained in another
component must equal it (both quasisimple subnormal), the `L ≤ M` case is excluded,
leaving `⁅L, M⁆ = 1`. The dichotomy is proved by induction on the subnormal length
of `H`; the base case `H ⊴ G` uses that `H ∩ L ⊴ L`, that `↥L` is quasisimple (so a
normal subgroup of `↥L` is central or all of `↥L`), and the Wielandt join to control
the normal closure. The three-subgroups lemma
(`commutator_commutator_eq_bot_of_rotate`) + perfectness `⁅L, L⁆ = L` close it.
-/

namespace AristotleTarget
...
/-- `H` is subnormal in `K`: a finite chain of normal steps. -/
def IsSubnormal (H K : Subgroup G) : Prop :=
  Relation.ReflTransGen IsNormalStep H K

/-- A quasisimple group: perfect, and simple modulo its center. -/
structure IsQuasisimple (Q : Type*) [Group Q] : Prop where
  perfect : commutator Q = ⊤
  simple_quotient : IsSimpleGroup (Q ⧸ Subgroup.center Q)

/-- A component of `G`: a subnormal subgroup whose carrier is quasisimple. -/
def IsComponent (K : Subgroup G) : Prop :=
  IsSubnormal K ⊤ ∧ IsQuasisimple K

/-- **Wielandt's join theorem** (true for finite groups). Supplied as an axiom you
may freely use: the join of two subnormal subgroups of a finite group is subnormal. -/
axiom isSubnormal_sup [Finite G] {H K : Subgroup G}
    (hH : IsSubnormal H ⊤) (hK : IsSubnormal K ⊤) : IsSubnormal (H ⊔ K) ⊤

/-! ## Helper lemmas -/

/-- Forward induction along `ReflTransGen`: if `P a` holds and `P` propagates
forward along every `r`-step (with access to the rest of the chain), then `P b`. -/
...
/-- `IsSubnormal H K` implies `H ≤ K`. -/
private lemma le_of_isSubnormal {H K : Subgroup G} (h : IsSubnormal H K) : H ≤ K := by
  induction h with
  | refl => exact le_refl _
  | tail _ step ih => exact le_trans ih step.1
...
/-- In a quasisimple group, a normal subgroup is either central or everything. -/
private lemma quasisimple_normal_le_center_or_eq_top {Q : Type*} [Group Q]
    (hQ : IsQuasisimple Q) (N : Subgroup Q) (hN : N.Normal) :
    N ≤ Subgroup.center Q ∨ N = ⊤ := by
  set Z := Subgroup.center Q
  set π := QuotientGroup.mk' Z
  haveI := hQ.simple_quotient
  have hπ : Function.Surjective π := QuotientGroup.mk'_surjective Z
  have hNimg_normal : (N.map π).Normal := Subgroup.Normal.map hN π hπ
  rcases IsSimpleGroup.eq_bot_or_eq_top_of_normal (N.map π) hNimg_normal with h | h
  · left
  ...
  · right
    have hsup : N ⊔ Z = ⊤ := by
      rw [eq_top_iff]
      intro q _
      have hq : π q ∈ N.map π := by rw [h]; trivial
      obtain ⟨n, hn, hneq⟩ := hq
      have hnz : n⁻¹ * q ∈ Z := by
        rwa [QuotientGroup.mk'_apply, QuotientGroup.mk'_apply, QuotientGroup.eq] at hneq
      rw [show q = n * (n⁻¹ * q) from by group]
      exact Subgroup.mul_mem _ ((le_sup_left : N ≤ N ⊔ Z) hn)
        ((le_sup_right : Z ≤ N ⊔ Z) hnz)
    haveI := hN
    have hcomm := Subgroup.Normal.commutator_le_of_self_sup_commutative_eq_top
      hsup (Subgroup.center.isMulCommutative Q)
    change commutator Q ≤ N at hcomm
    rw [hQ.perfect] at hcomm
    exact eq_top_iff.mpr hcomm
...
/-- Key containment: if `L ≤ M`, `IsNormalStep M J`, `IsNormalStep H K`,
`J ≤ K`, then `⁅L, H ⊓ J⁆ ≤ H ⊓ M`.
Elements `[l, h]` land in `M` (by normality of `M` in `J`) and in `H`
(by normality of `H` in `K`). -/
...
/-- Centralizing propagates along the subnormal chain: if `L ◁◁ K`, `H ◁ K`,
`⁅L, L⁆ = L`, and `⁅L, H ⊓ L⁆ = ⊥`, then `⁅L, H⁆ = ⊥`.
Proved by forward induction: at each step `M ◁ J` in the chain from `L` to `K`,
`⁅L, H ⊓ J⁆ ≤ H ⊓ M` and the three-subgroups lemma + perfectness gives
`⁅L, H ⊓ J⁆ = ⊥` from `⁅L, H ⊓ M⁆ = ⊥`. -/
private lemma centralizing_by_subnormal {L K H : Subgroup G}
  ...
  exact (reflTransGen_forward hsub
    ...
    ⟨le_refl L, le_of_isSubnormal hsub, hbase⟩
    (fun M J hMJ hJK ⟨hLM, _, hIH⟩ => by
      have hJK' : J ≤ K := le_trans (le_of_isSubnormal hJK) le_rfl
...
/-- Component-vs-subnormal dichotomy: a component `L` and a subnormal subgroup `H`
satisfy `L ≤ H` or `⁅L, H⁆ = ⊥`.
Proved by induction on the subnormal chain of `H` in `⊤`. -/
private lemma component_subnormal_dichotomy [Finite G] {L H : Subgroup G}
  ...
  | head hHK _hKT ih =>
    ...
    · -- L ≤ K: analyze H ⊓ L inside L
      have hN := inf_subgroupOf_normal_of_normalStep hHK hLK
      rcases quasisimple_normal_le_center_or_eq_top hL.2 _ hN with hcent | htop
      · -- H ⊓ L central in L: use three-subgroups + perfectness
        right
        have hperf := perfect_commutator_eq_self hL.2.perfect
        have hbase := commutator_eq_bot_of_subgroupOf_le_center hcent
        have hsub := isSubnormal_of_le hL.1 hLK
        exact centralizing_by_subnormal hsub hHK hperf hbase
      · -- H ⊓ L = L as subgroup: L ≤ H
        left; exact le_of_subgroupOf_eq_top htop
    · -- ⁅L, K⁆ = ⊥: ⁅L, H⁆ ≤ ⁅L, K⁆ = ⊥ by monotonicity
      right
      exact le_bot_iff.mp (le_trans (Subgroup.commutator_mono le_rfl hHK.1) (le_of_eq hLK_bot))

/-- **GOAL.** Distinct components of a finite group commute: `⁅L, M⁆ = ⊥`.
(Aschbacher, *Finite Group Theory*, 31.4.) -/
theorem components_commute [Finite G] {L M : Subgroup G}
    (hL : IsComponent L) (hM : IsComponent M) (hne : L ≠ M) :
    ⁅L, M⁆ = ⊥ := by
  rcases component_subnormal_dichotomy hL hM.1 with hLM | h
  · rcases component_subnormal_dichotomy hM hL.1 with hML | h
    · exact absurd (le_antisymm hLM hML) hne
    · rwa [Subgroup.commutator_comm] at h
  · exact h
