...
## F. ▶ NEXT THREAD — `PSp_{2n}(q)` simplicity via symplectic Iwasawa (started 2026-06-04)

With PSL fully closed, the natural continuation is the next classical family. **`PSp n q`
is already concrete** in `LieType.lean`: `symplecticGroup (Fin n) (ZMod q) ⧸ center`
(mathlib's `Matrix.symplecticGroup`, form `J = [[0,1],[-1,0]]` on `(Fin n ⊕ Fin n)`).
`axiom PSp_isSimpleGroup (n q) (2 ≤ n) (¬(n=2∧q=2))` is the target. This mirrors the PSL
Iwasawa arc but the geometry is symplectic (transvections are `τ_{v,c}: x ↦ x + c·ω(x,v)·v`,
action on ℙ²ⁿ⁻¹). It is a fresh multi-lap arc (mathlib's `symplecticGroup` is minimal: only
`J`, `mem_iff`, inverse — NO transvections / center / generation / simplicity).

**Iwasawa obligations to build (each a brick), mirroring SLn:**
1. **Perfect:** `Sp(2n,F)` generated by symplectic transvections ⇒ `commutator = ⊤`.
   - Brick F.1a (FOUNDATION, **OUT at Aristotle `dab8c286`/`sptrans`**): define
     `spTransvection v c := 1 + c • vecMulVec v (J·v)` and prove `spTransvection_mem` (it lies
     in `symplecticGroup`). Math verified by hand: `MJMᵀ=J` since `JᵀJ=1`, `JNᵀ=-NJ`,
     `vᵀJv=0`. Also `det = 1` (so it's in SL too). Harvest → port → new file `SpTransvection.lean`.
   - Brick F.1b (HARD): **symplectic transvections generate `Sp(2n,F)`** (the perfectness input;
     symplectic Witt / Eichler). The analog of `transvecSL_closure_eq_top`. May need mathlib's
     symplectic Witt infrastructure (check existence first; likely absent → a big brick).
   - Brick F.1c: each symplectic transvection is a commutator (for `n ≥ 2`) ⇒ perfect.
2. **MulAction / faithful:** `PSp ↷ ℙ²ⁿ⁻¹` via `mulVec` descent through center (reuse the
   `SLnAction` pattern — but Sp center is `{±1}`, characterize via `symplecticGroup` center).
3. **Quasi-preprimitive:** `Sp` is **transitive on nonzero vectors / on ℙ²ⁿ⁻¹** (Witt's thm:
   one orbit since the form is non-degenerate alternating) — but NOT 2-transitive (preserves
   `ω(x,y)=0`); primitivity needs the maximal-parabolic argument, NOT 2-transitivity. **This is
   the key structural difference from PSL** — do not blindly copy `exists_sl_maps_two_points`.
4. **IwasawaStructure:** the transvection subgroup of a point (isotropic line) stabilizer —
   `{τ_{v,c} : c ∈ F}` for fixed `v` is abelian, conjugation-equivariant, and generates.

**First concrete step next lap:** harvest `dab8c286` (spTransvection_mem), create
`FiniteSimpleGroups/SpTransvection.lean` with the def + membership + `det=1`, then attack F.1b.
Reference: Grove "Classical Groups and Geometric Algebra", Artin "Geometric Algebra" (symplectic
transvections generate Sp). **Aristotle in flight: `dab8c286` (sptrans).**
